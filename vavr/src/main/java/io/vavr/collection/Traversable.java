...
public interface Traversable<T> extends Foldable<T>, Value<T> {
    ...
    @Override
    default <U> U foldLeftWhile(U zero, BiFunction<? super U, ? super T, ? extends U> combine, Predicate<U> validator) {
        Objects.requireNonNull(combine, "f is null");
        Objects.requireNonNull(validator, "validator is null");
        U xs = zero;
        for (T x : this) {
            if (!validator.test(xs)) {
                return xs;
            }
    ...
    @Override
    default <U> U foldLeftUntil(U zero, BiFunction<? super U, ? super T, ? extends U> combine, Predicate<U> terminator) {
        Objects.requireNonNull(combine, "f is null");
        Objects.requireNonNull(terminator, "terminator is null");
        U xs = zero;
        for (T x : this) {
            if (terminator.test(xs)) {
                return xs;
            }
            xs = combine.apply(xs, x);
        }
